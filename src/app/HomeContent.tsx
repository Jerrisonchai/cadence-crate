...
// Sample data for development — will be replaced by Supabase API
...
export default function HomeContent() {
  ...
  const filteredSongs = useMemo(() => {
    ...
    if (activeDecade) {
      ...
      result = result.filter(
        (s) =>
          s.release_year &&
          s.release_year >= startYear &&
          s.release_year < startYear + 10
      );
    }
  ...
  return (
    <>
      <Header totalSongs={SAMPLE_SONGS.length} />
      <FilterChips
        ...
        {visibleSongs.length > 0 ? (
            ...
            {hasMore && (
              <div className="mt-8 text-center">
                <button
                  ...
                  className="rounded-xl border border-border bg-surface/50 px-8 py-3 font-display text-sm font-medium text-text-secondary backdrop-blur-md transition-all hover:border-pulse/30 hover:text-pulse"
                >
                  Load More Songs
                </button>
          ...
          <div className="flex flex-col items-center justify-center py-24">
            ...
            <h3 className="mb-2 font-display text-lg font-semibold text-text-primary">
              No songs in this filter yet
            </h3>
            <p className="mb-6 max-w-md text-center font-body text-sm text-text-secondary">
              We&apos;re building the library week by week. The BPM Collector
              runs every Sunday. Check back soon!
            </p>
            <button
              onClick={() => {
